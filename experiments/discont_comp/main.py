...
if __name__ == "__main__":
    ...
    # for D in range(2,20):
    D = 2
    for num_obs in range(5,11):
        print(D)
        np.random.seed(0)
        env = NavigateGTEnv(B, D, test_energy2d, comp_device, control='force', sparse_r_step=H-1, dt=dt, obstacles_env=True, num_obs = num_obs)
        print("Running grad+cem planner")
        m, std = run_mult(gradcem_planner, env,50)
        gradcem_return.append(m)
        gradcem_std.append(std)

        np.random.seed(0)
        env = NavigateGTEnv(B, D, test_energy2d, comp_device, control='force', sparse_r_step=H-1, dt=dt, obstacles_env=True, num_obs = num_obs)
        print("Running grad planner")
        m, std = run_mult(grad_planner, env,50)
        ...
        m, std = run_mult(cem_planner, env,50)
        cem_return.append(m)
        cem_std.append(std)
    ...
    np.save("results_new_num_obs1.npy", results)
